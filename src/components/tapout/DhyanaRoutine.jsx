...
export default function DhyanaRoutine({ mode: initialMode, onDone, onBack }) {
  ...
  const content = isMorning ? [
    ...
    { title: "Letting Go", text: "List 1-3 tasks you will not think about until tomorrow.", type: "letting_go" },
    { title: "Mind Dump", text: "Anything else on your mind before sleep? (Optional)", type: "journal" }
  ];
  ...
  const textColor = isMorning ? "text-[#3E2723]" : "text-[#FDFBF7]";
  const subTextColor = isMorning ? "text-[#5D4037]" : "text-[#C8C2D8]";
  ...
  return (
    <div className={`flex flex-col h-full animate-fade-in relative transition-colors duration-700`} style={{ background: bgStyle }}>
      ...
      <div className="flex items-center justify-between px-5 py-4 z-10 relative">
        ...
        <button onClick={handleToggle} className="p-2 rounded-full bg-black/10 backdrop-blur-sm shadow-inner transition-transform active:scale-95">
          {isMorning ? <Moon size={18} className={textColor} /> : <Sun size={18} className={textColor} />}
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col px-6 pb-8 justify-center items-center text-center z-10 relative">
        ...
        {current.type === "breathing" && (
          <div className="relative flex items-center justify-center h-48 mb-8">
            ...
            >
              <p className={`text-sm font-bold tracking-widest uppercase ${textColor}`}>{breathPhase}</p>
            </div>
          </div>
        )}
        ...
        {current.type === "boundary" && (
          <div className="flex flex-col gap-3 mt-8 w-full">
            {["No work apps after 9pm", "Take lunch away from my desk", "Leave office on time"].map((b) => (
              <button key={b} onClick={() => setBoundary(b)} className={`p-4 rounded-2xl text-sm font-semibold transition-all text-left ${boundary === b ? "bg-white text-[#FF9B71] shadow-lg scale-105" : `${inputBg} ${textColor}`}`}>{b}</button>
            ))}
          </div>
        )}

        {(current.type === "text" || current.type === "letting_go" || current.type === "journal") && (
          <div className="w-full mt-6 flex flex-col gap-2">
            <textarea
              value={current.type === "text" ? priorities : current.type === "letting_go" ? letGoText : journalText}
              onChange={(e) => current.type === "text" ? setPriorities(e.target.value) : current.type === "letting_go" ? setLetGoText(e.target.value) : setJournalText(e.target.value)}
            ...
            {current.type === "letting_go" && letGoText.length > 5 && (
               <p className={`text-xs mt-2 ${subTextColor} animate-fade-in`}>✓ Noted. They belong to tomorrow-you.</p>
            )}
            {current.type === "journal" && (
               <p className={`text-xs mt-2 ${subTextColor} flex items-center justify-center gap-1`}><Lock size={12}/> Encrypted</p>
            )}
          </div>
        )}
        ...
        {current.type === "review" && (
          <div className="flex flex-col gap-3 mt-6 w-full">
            <div className="flex gap-3">
              {["Yes", "Partly", "No"].map((r) => (
                <button key={r} onClick={() => setBoundaryHonoured(r)} className={`flex-1 py-4 rounded-2xl text-sm font-semibold transition-all ${boundaryHonoured === r ? "bg-white text-[#1B2845] shadow-lg scale-105" : `${inputBg} ${textColor}`}`}>{r}</button>
              ))}
            </div>
            {boundaryHonoured && boundaryHonoured !== "Yes" && (
               <input type="text" placeholder="What got in the way? (e.g., client emergency)" className={`w-full p-4 mt-2 rounded-2xl text-sm outline-none backdrop-blur-sm ${inputBg} ${textColor} animate-fade-in placeholder-opacity-50`} />
            )}
          </div>
        )}
      ...
      <div className="px-6 pb-8 z-10 relative">
        <button onClick={isLast ? onDone : () => setStep(s => s + 1)} className={`w-full py-4 rounded-full font-bold flex items-center justify-center gap-2 shadow-xl backdrop-blur-md transition-all active:scale-95 ${isMorning ? "bg-white/80 text-[#3E2723] hover:bg-white" : "bg-[#6D7BFF] text-white hover:bg-[#5560EE]"}`}>
