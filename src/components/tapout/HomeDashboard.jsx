    ...
    <div className="flex flex-col h-full px-5 pt-2 pb-4 gap-3" style={{ background: "#E2DEF2" }}>
      ...
      {hasRedZone && (
        <button onClick={onRedZone} className="flex items-center gap-3 p-3 rounded-2xl animate-fade-in" style={{ background: "#FFEDED", border: "1px solid #FF6B6B" }}>
          ...
          <div className="flex-1 text-left"><p className="text-xs font-semibold" style={{ color: "#E8343A" }}>Red Zone Pattern Detected</p><p className="text-[10px] leading-tight" style={{ color: "#4A4868" }}>3+ high-stress days this week. Tap to view.</p></div>
      ...
      {!snapshotSaved && (
        <button
          onClick={onLogDay}
          className="rounded-[2rem] p-2 pr-4 flex items-center gap-4 animate-fade-in bg-white border-2 border-[#E2DEF2] hover:border-[#B8B8FF] transition-colors w-full shadow-sm"
        >
          <div className="w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 bg-[#6D7BFF]">
            <Moon size={20} strokeWidth={1.5} className="text-white" />
          </div>
          <div className="flex-1 text-left">
            <p className="text-sm font-bold text-[#003B64]">Nightly check-in ready</p>
            <p className="text-[11px] text-[#B7C4D8] mt-0.5">How was your workday? (1 min)</p>
          </div>
          <div className="text-xs font-bold px-4 py-2 rounded-full bg-[#6D7BFF] text-white">
            Log
          </div>
        </button>
      )}
